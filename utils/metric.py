...
def compute_metric(sketch_vectors, sketch_labels, image_vectors, image_labels):
    ...
    map_all, p_100 = sake_metric(image_vectors, image_labels, sketch_vectors, sketch_labels)
    map_200, p_200 = sake_metric(image_vectors, image_labels, sketch_vectors, sketch_labels,
...
def sake_metric(predicted_features_gallery, gt_labels_gallery, predicted_features_query, gt_labels_query, k=None):
    ...
    mAP = map_sake(gt_labels_gallery, predicted_features_query, gt_labels_query, scores, k=k['map'])
    Prec = prec_sake(gt_labels_gallery, predicted_features_query, gt_labels_query, scores, k=k['precision'])

    return sum(mAP) / len(mAP), Prec
