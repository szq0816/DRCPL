...
class ResidualAttentionBlock_DRCPL(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.compound_prompt_nctx = design_details['drcpl_length']  # 2
    ...
    def forward(self, inputs):
            ...
            if len(compound_prompts_deeper) > 0:
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        visual_context = visual_context.expand(num, -1, -1).permute(1, 0, 2)
        ...
        if not self.text_layer:
            return [x, compound_prompts_deeper, counter, num]
        else:
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'DRCPL':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_DRCPL(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
            # Corresponds to default ZsClip
            assert current_trainer == 'ZsClip'
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        if self.VPT_shallow:
            ...
            n_ctx = design_details["vision_ctx"]  # hyper parameter
        ...
        # hyperparameter if you need to add prompt embeddings inside to the input
    ...
    def forward(self, x: torch.Tensor):
        ...
        # After positional embeddings, we will attach prompts with the models, remember only those
...
class VisionTransformer_DRCPL(nn.Module):
    ...
    def forward(self, x: torch.Tensor, first_layer_prompts, visual_deep_prompts):
        num = x.shape[0]
        ...
        # are trainable parameters here in whole image encoder.
        if self.VPT_shallow:
            visual_ctx = first_layer_prompts.expand(num, -1, -1)
            x = torch.cat([x, visual_ctx], dim=1)
        else:
            assert self.prompt_till_layer_visual == 0
        ...
        outputs = self.transformer([x, visual_deep_prompts, 0, num])
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "DRCPL":
                self.visual = VisionTransformer_DRCPL(
                    input_resolution=image_resolution,
...
def convert_weights(model: nn.Module):
    """Convert applicable models parameters to fp16"""
...
def build_model(state_dict: dict, design_details):
    ...
    try:
        ...
        print('Weights not found for some missing keys: ', len(missing_keys))
    return model.eval()
